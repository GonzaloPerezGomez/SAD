...
def train(k, K, p, datos):
    ...
    for indexK in range(k, K+1):
        for indexP in range(1, p+1):
            for w in weights:
                y_test, y_pred = kNN.kNN(datos, indexK, w, indexP)
                fscore_micro, fscore_macro = calculate_fscore(y_test, y_pred)
                gen_informe(indexK, indexP, w, fscore_micro, fscore_macro)

def gen_informe(indexK, indexP, w, fscore_micro, fscore_macro):
    with open('informe.csv', 'a', newline='') as csvfile:
        spamwriter = csv.writer(csvfile)
        spamwriter.writerow(["K = " + str(indexK), "P = " + str(indexP), w, "Micro = " + str(fscore_micro), "Macro = " + str(fscore_macro)])
